...
me2amu = 9.1093837015E-31 / 1.66053906660E-27 # electron mass to atomic mass unit (me/mu , CODATA 2018)
	...
	while (theta < thetamax):
		while (psi < psimax):
			while (phi < phimax):
				...
				if(psitype=='avg' and phitype=='avg'):	# twist+azimuthal average (psi+phi angles)
...
#FIXME TODO:include other options
	...
	beta = np.zeros(3)
	beta[0] = np.arcsin( (visfreq*np.sin(visangle) + irfreq*np.sin(irangle)) / sfgfreq)		# phase-matching condition [Eq. 4]
	beta[1] = visangle
	beta[2] = irangle
	...
	for i in range(3):
		...
		Lxx[i] = 2. * n1[i] * np.cos(gamma_i) / (n1[i] * np.cos(gamma_i) + n2[i] * np.cos(beta[i]))
		Lyy[i] = 2. * n1[i] * np.cos(beta[i]) / (n1[i] * np.cos(beta[i]) + n2[i] * np.cos(gamma_i))
		Lzz[i] = 2. * n2[i] * np.cos(beta[i]) / (n1[i] * np.cos(gamma_i) + n2[i] * np.cos(beta[i])) * (n1[i] / n3[i])** 2
...
# TODO: include mix polarizations
	...
	for i in range(len(modes)):
		if (sfgpol == 'ppp'):
			temp = Lijk[2,0,0] * chi[i][2,0,0] + Lijk[2,2,2] * chi[i][2,2,2] - Lijk[0,0,2] * chi[i][0,0,2] - Lijk[0,2,0] * chi[i][0,2,0]
		elif (sfgpol == 'ssp'):
...
def save_chi_data():
	...
	ofile.write('# {:4} {:10} {:18} {:18} \n'.format('mode','freq','A.real','A.imag'))
	for i in range(len(modes)):
		ofile.write('{:4} {:8.2f} {:12.6f} {:12.6f} \n'.format(modes[i],freq[i],chi_amplitude[i].real,chi_amplitude[i].imag))
	ofile.close()
	...
	for i in range(len(freqaxis)):	# loop over w_ir
		...
		ofile.write('{:8.2f}   {:1.4e}   {:1.4e}   {:1.4e} \n'.format(freqaxis[i],abs(temp)**2,temp.imag,temp.real))
	ofile.close()
...
def print_data(filename):
	...
	for i in range(len(modes)):
		...
		if(iverbose>=3 and sfgtype!='pmirras'):
			...
			for j in range(3):
				for k in range(3):
					ofile.write('\n {} {} {}'.format(beta[i][j][k][0], beta[i][j][k][1], beta[i][j][k][2]))
		ofile.write('\n')
	ofile.write('\n')
	...
	if(iverbose>=2 and sfgtype!='pmirras'):
		...
		for i in range(3):
			for j in range(3):
				ofile.write('\n {:.4f} {:.4f} {:.4f}'.format(Lijk[i][j][0], Lijk[i][j][1], Lijk[i][j][2]))
			ofile.write('\n')
		ofile.write('\n')
		ofile.write('----------------------')
...
def read_dipole_logfile(lines):
	...
	for i in range(len(lines)):
		if (label in lines[i]):
			ierr = 0
			temp = lines[i].split()
			temp1 = float(temp[5].replace('D','E'))   #replace exponential 'D' notation with 'E'
	...
	for i in range(len(dipole)):	 
		dipole[i] /= np.sqrt(2.*freq[i]) 

	### convert units
	fct = 1./42.2561	# 42.2561 km/mol = Debye^2 / \AA^2 / amu
	for i in range(len(dipole)):	 
	...
	for i in range(len(lines)):
		if (label in lines[i]):
			...
			for j in range(3):
				...
				temp1 = float(temp[1].replace('D','E'))    #replace exponential 'D' notation with 'E'
